...
class Controller {
 public:
  ...
  void run() {
    ...
    // Match the official writer: no command is published until the first
    // LowState arrives and INIT creates the standing ramp command.
    std::thread writer([this]{write_loop();});
    std::thread key_reader([this]{
      char key = 0;
      while (!stop_ && std::cin.get(key)) {
        if (key == ']') start_requested_ = true;
        if (key == 't' || key == 'T') play_requested_ = true;
      }
    });
    init_loop();
    control_loop();
